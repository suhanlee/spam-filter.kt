...
class F28 {
    fun isSpam(message: String): Boolean {
        ...
        val textLower = message.toLowerCase()
        ...
        val urls = urlRegex.findAll(message).toList()
        if (urls.isNotEmpty()) {
            return true
        }
        ...
        for (pattern in unusualPatterns) {
            if (pattern.find(message) != null) {
                return true
            }
        }

        // Check for sequences of numbers and characters combined
        val sequences = "([0-9]+[a-zA-Z]+|[a-zA-Z]+[0-9]+)".toRegex().findAll(message).toList()
        if (sequences.size > 1) {
            return true
        }
