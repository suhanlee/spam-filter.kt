...
class F27 {
    fun isSpam(message: String): Boolean {
        ...
        val digits = Regex("""\d{3,}""").findAll(message).map { it.value }.toList()
        val percentages = Regex("""\d{2,}%+""").findAll(message).map { it.value }.toList()
        if (digits.isNotEmpty() || percentages.isNotEmpty()) {
            return true
        }

        // Check for multiple special characters
        val specialChars = Regex("""[\*-_@.&+:]+""").findAll(message).map { it.value }.toList()
